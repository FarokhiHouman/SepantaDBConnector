...
public static class ClsRegEdit {
    ...
    public static string[] GetAllCompanies() =>
        Registry.CurrentUser.OpenSubKey(name: Resources.reg_Software, writable: true).OpenSubKey(name: Resources.reg_Sepanta, writable: true).
                 GetSubKeyNames();
    public static string GetCompanyDescription(string companyName) =>
        Convert.ToString(value: Registry.CurrentUser.OpenSubKey(name: Resources.reg_Software, writable: true).
                                         OpenSubKey(name: Resources.reg_Sepanta, writable: true).OpenSubKey(name: companyName, writable: true).
    ...
    public static bool CreateCompany(string companyName, string companyDesc) {
        if (ClsRegEdit.CompanyExist(companyName: companyName))
            return false;
        Registry.CurrentUser.OpenSubKey(name: Resources.reg_Software, writable: true).OpenSubKey(name: Resources.reg_Sepanta, writable: true).
                 CreateSubKey(subkey: companyName).SetValue(name: Resources.reg_Description, value: companyDesc, valueKind: RegistryValueKind.String);
        return true;
    }
    private static bool CompanyExist(string companyName) => ClsRegEdit.GetAllCompanies().Any(predicate: company => company == companyName);
    ...
    public static bool RemoveApplication(string selectedCompany, string selectedApplication) {
        if (!ClsRegEdit.AppExist(appName: selectedApplication, companyName: selectedCompany))
            return false;
        Registry.CurrentUser.OpenSubKey(name: Resources.reg_Software, writable: true).OpenSubKey(name: Resources.reg_Sepanta, writable: true).
                 OpenSubKey(name: selectedCompany, writable: true).DeleteSubKeyTree(subkey: selectedApplication);
        return true;
    }
    public static string GetAppVersion(string selectedCompany, string selectedApplication) =>
        Registry.CurrentUser.OpenSubKey(name: Resources.reg_Software, writable: true).OpenSubKey(name: Resources.reg_Sepanta, writable: true).
                 OpenSubKey(name: selectedCompany, writable: true).OpenSubKey(name: selectedApplication, writable: true).
    ...
    public static bool AppHasConnection(string app, string company) {
        string[] values = Registry.CurrentUser.OpenSubKey(name: Resources.reg_Software, writable: true).
                                   OpenSubKey(name: Resources.reg_Sepanta, writable: true).OpenSubKey(name: company, writable: true).
    ...
    public static string GetConeectionString(string app, string company) =>
        Registry.CurrentUser.OpenSubKey(name: Resources.reg_Software, writable: true).OpenSubKey(name: Resources.reg_Sepanta, writable: true).
                 OpenSubKey(name: company, writable: true).OpenSubKey(name: app, writable: true).GetValue(name: Resources.reg_CentralConnection).
                 ToString();
