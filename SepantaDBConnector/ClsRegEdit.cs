...
public static class ClsRegEdit
{
	...
	{
		string[] regKeys =
			Registry.CurrentUser.OpenSubKey(name: Resources.reg_Software, writable: true)!.GetSubKeyNames();
		return regKeys.Any(predicate: regKey => regKey == Resources.reg_Sepanta);
	}

	public static string[] GetAllCompanies() => Registry.CurrentUser
	...
	{
		if (CompanyExist(companyName: companyName))
			Registry.CurrentUser.OpenSubKey(name: Resources.reg_Software, writable: true)
				.OpenSubKey(name: Resources.reg_Sepanta, writable: true).DeleteSubKeyTree(subkey: companyName);
	}
	...
	public static bool RemoveApplication(string selectedCompany, string selectedApplication)
	{
		...
		Registry.CurrentUser.OpenSubKey(name: Resources.reg_Software, writable: true)
			.OpenSubKey(name: Resources.reg_Sepanta, writable: true).OpenSubKey(name: selectedCompany, writable: true)
