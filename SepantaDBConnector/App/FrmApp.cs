...
public partial class FrmApp : XtraForm
{
	...
	private void btnAdd_Click(object sender, EventArgs e)
	{
		...
		{
			MessageBox.Show(text: Resources.NameEmpty, caption: string.Empty, buttons: MessageBoxButtons.OK,
	...
	private void btnRemove_Click(object sender, EventArgs e)
	{
		...
		{
			MessageBox.Show(text: Resources.NameEmpty, caption: "", buttons: MessageBoxButtons.OK,
				icon: MessageBoxIcon.Error);
			return;
		}

		if (MessageBox.Show(text: Resources.AreYouSure, caption: "", buttons: MessageBoxButtons.YesNo,
				icon: MessageBoxIcon.Question) == DialogResult.No) return;
		if (!ClsRegEdit.RemoveApplication(selectedCompany: SelectedCompany,
			selectedApplication: SelectedApplication)) return;
		FrmApp_Load(sender: sender, e: e);
	}
	...
	private void lstApp_SelectedIndexChanged(object sender, EventArgs e)
	{
		...
		txtAppVersion.Text =
			ClsRegEdit.GetAppVersion(selectedCompany: SelectedCompany, selectedApplication: SelectedApplication);
		txtAppDescription.Text =
			ClsRegEdit.GetAppDesc(selectedCompany: SelectedCompany, selectedApplication: SelectedApplication);
	}
}