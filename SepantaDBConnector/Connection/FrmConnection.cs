...
#region Other
	...
	private void btnTestConnection_Click(object sender, EventArgs e)
	{
		...
		{
			sqlConnection.Open();
			MessageBox.Show(text: Resources.Con_TestSucseeded, caption: Resources.SepantaConnection, buttons: MessageBoxButtons.OK, icon: MessageBoxIcon.Asterisk);
	...
	private void btnSaveConnection_Click(object sender, EventArgs e)
	{
		try
		{
			...
			MessageBox.Show(text: Resources.ConnectionSaved, caption: Resources.SepantaConnection, buttons: MessageBoxButtons.OK, icon: MessageBoxIcon.Asterisk);
		}
		catch (Exception)
		{
			MessageBox.Show(text: Resources.ConnectionNotSaved, caption: Resources.SepantaConnection, buttons: MessageBoxButtons.OK, icon: MessageBoxIcon.Error);
		}
	}
	private void cmbApplication_SelectedIndexChanged(object sender, EventArgs e)
	{
		...
		{
			grpDataase.Enabled          = grpLogin.Enabled = grpDataSource.Enabled = grpTestSaveBtn.Enabled = btnDeleteConnection.Enabled = false;
			btnCreateConnection.Enabled = true;
		}
		else
		{
			grpDataase.Enabled          = grpLogin.Enabled = grpDataSource.Enabled = grpTestSaveBtn.Enabled = btnDeleteConnection.Enabled = true;
			btnCreateConnection.Enabled = false;
			_connectionStringBuilder    = new() { ConnectionString = ClsRegEdit.GetConeectionString(app: SelectedApplication, company: SelectedCompany) };
	...
	private void btnDeleteConnection_Click(object sender, EventArgs e)
	{
		...
		{
			MessageBox.Show(text: exception.Message, caption: "", buttons: MessageBoxButtons.OK, icon: MessageBoxIcon.Warning);
		}
	}
#endregion
#endregion
